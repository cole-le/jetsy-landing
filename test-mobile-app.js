...
// Mobile App Landing Page Quality Test
// This script tests the quality and completeness of mobile app landing pages

const API_BASE = 'http://localhost:8787';

async function testMobileApp() {
  console.log('📱 Testing Mobile App Landing Page Quality...\n');
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/llm-orchestrate`, {
      ...
      body: JSON.stringify({
        project_id: `mobile-app-test-${Date.now()}`,
...
function analyzeCodeQuality(code, testCase) {
  ...
  testCase.expectedForms.forEach(form => {
    ...
    if (formLower.includes('beta signup')) {
      found = codeLower.includes('beta') || codeLower.includes('signup') || codeLower.includes('sign up') || 
              (codeLower.includes('form') && (codeLower.includes('email') || codeLower.includes('input')));
    } else {
  ...
  testCase.expectedFeatures.forEach(feature => {
    ...
    if (featureLower.includes('app preview') || featureLower.includes('screenshots')) {
      found = codeLower.includes('screenshot') || codeLower.includes('mockup') || codeLower.includes('app preview') || codeLower.includes('phone') || codeLower.includes('mobile');
    } else if (featureLower.includes('feature showcase')) {
      found = codeLower.includes('feature') && (codeLower.includes('showcase') || codeLower.includes('grid') || codeLower.includes('card'));
    } else if (featureLower.includes('beta signup')) {
      found = codeLower.includes('beta') || codeLower.includes('signup') || codeLower.includes('sign up') || codeLower.includes('email');
    } else if (featureLower.includes('download cta')) {
      found = codeLower.includes('download') || codeLower.includes('app store') || codeLower.includes('google play') || codeLower.includes('get app');
    } else if (featureLower.includes('mobile-focused')) {
      found = codeLower.includes('mobile') || codeLower.includes('responsive') || codeLower.includes('app');
    } else if (featureLower.includes('app store badges')) {
      found = codeLower.includes('app store') || codeLower.includes('google play') || codeLower.includes('badge') || codeLower.includes('download');
    } else if (featureLower.includes('user testimonials')) {
      found = codeLower.includes('testimonial') || codeLower.includes('review') || codeLower.includes('customer') || codeLower.includes('user');
    } else {
